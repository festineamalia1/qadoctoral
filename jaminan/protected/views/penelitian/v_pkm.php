...
?>
		...
		<p>
			...
			di lingkungan Fakultas dalam lima tahun terakhir dengan mengikuti format tabel berikut :
		</p>
		<table class="table table-bordered min">
			<thead>
				<tr>
					...
					<th rowspan="1" colspan="5">Jumlah Judul Kegiatan Pelayanan/Pengabdian kepada Masyarakat</th>
					<th rowspan="2" colspan="3">Total Dana Kegiatan Pelayanan/Pengabdian kepada Masyarakat</th>
				</tr>
				<tr>
...
?>
		<p class="judul" style="font-weight:bold;">
			7.2   Kegiatan Pelayanan/Pengabdian kepada Masyarakat (PkM) dalam Lima Tahun Terakhir

		</p>
		<p>7.2.1  Tuliskan jumlah kegiatan pelayanan/pengabdian kepada masyarakat (*) yang sesuai dengan bidang keilmuan PS yang dilakukan oleh dosen tetap yang bidang keahliannya sesuai dengan PS dalam lima tahun terakhir dengan mengikuti format tabel berikut:</p><br>
		<?php
		if(empty($data)){
			?>
				...
				if(isset($data['sendiri'])){
					...
					foreach ($data['sendiri'] as $key => $value) {
						...
						?>
							...
							$jumlah=+$value['waktu_ts4']+$value['waktu_ts2']+$value['waktu_ts1']+$value['waktu_ts'];
						?>	
							<td style="text-align:center"><?=$value['waktu_ts4']?></td>
							<td style="text-align:center"><?=$value['waktu_ts3']?></td>
				...
				if(isset($data['pt'])){
					...
					foreach ($data['pt'] as $key => $value) {
						...
						if($i == 1){
							...
							$jumlah=$value['waktu_ts4']+$value['waktu_ts3']+$value['waktu_ts2']+$value['waktu_ts1']+$value['waktu_ts'];
						?>	
							<td style="text-align:center"><?=$value['waktu_ts4']?></td>
							<td style="text-align:center"><?=$value['waktu_ts3']?></td>
							<td style="text-align:center"><?=$value['waktu_ts2']?></td>
		...
		<div class="row-fluid">
			<div class="span12" style="text-align:right;">
				<a href="<?=Yii::app()->getBaseUrl(true)?>/penelitian/CetakPDFpkm/?id_prodi=<?=$id_prodi?>&id_administrasi=<?=$id_administrasi?>" target="_blank" class="btn btn-primary" value="Cetak Borang"> <i class="icon-print icon-white"></i> Cetak Borang</a>
			</div>
		</div>
<?
}
