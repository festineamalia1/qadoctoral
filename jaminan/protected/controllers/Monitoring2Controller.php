...
class Monitoring2Controller extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Monitoring2']))
		{
			$model->attributes=$_POST['Monitoring2'];
			if($model->save()){
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('Monitoring2');
		array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$this->layout='//layouts/column2';
		$model=new Monitoring2('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Monitoring2']))
			$model->attributes=$_GET['Monitoring2'];
	...
	{
		$model=Monitoring2::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Monitoring2 $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='monitoring2-form')
		{
	...
	public function actionGetData_monitoringevaluasi(){
		...
		$this->render('v_getdata_monitoring2',$data);
	}
	...
	public function actionSentData(){
	    ...
	    ->select('t.id_monitoring,t.pelaksanaan_ujian,t.usul_penelitian,t.penulisan_disertasi,t.kelayakan_dosen,t.ujian_akhir,t.lampiran,p.sumber_data')
	    ->from('monitoringevaluasi t')
		...
		$myHtml = $this->renderPartial('v_monitoringe2',$data,true); 
		echo $myHtml;
		Yii::app()->end(); 
		return;	
	}	

	public function actionCetakPDFMonitoring_2(){
		...
		$data['data'] = array();
		$data['data'] = Yii::app()->db->createCommand()
	    ->select('t.id_prodi,t.pelaksanaan_ujian,t.usul_penelitian,t.penulisan_disertasi,t.kelayakan_dosen,t.ujian_akhir,t.lampiran')
	    ->from('monitoringevaluasi t')
	    ->join('prodi p', 't.id_prodi=p.id_prodi')
	    ->join('administrasi a', 't.id_administrasi=a.id_administrasi')
	    ->andWhere('t.id_prodi=:id', array(':id'=>$id_prodi))
        ...
        if($id_prodi == 1){
        	...
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_monitoring_2', $data, true));
        }
