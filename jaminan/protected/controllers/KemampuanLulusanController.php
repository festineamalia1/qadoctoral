...
class KemampuanLulusanController extends Controller
{
	/**
	...
	{
		$this->layout='//layouts/column2';
		$model=new KemampuanLulusan;


		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['KemampuanLulusan']))
		{
			$model->attributes=$_POST['KemampuanLulusan'];
			if($model->save()){
				$this->redirect(array('view','id'=>$model->id_kemampuan_lulusan));
		}
}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('KemampuanLulusan');
		array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$this->layout='//layouts/column2';
		$model=new KemampuanLulusan('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['KemampuanLulusan']))
			$model->attributes=$_GET['KemampuanLulusan'];
	 ...
	 * @return KemampuanLulusan the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=KemampuanLulusan::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param KemampuanLulusan $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='kemampuan-lulusan-form')
		{
...
public function actionGetDatakemampuanlulusan(){
	...
	public function actionCetakPDFKemampuanLulusan(){
		...
		$data['data'] = array();
		$data['data'] = Yii::app()->db->createCommand()
	    ->select('t.id_prodi,t.pelaksanaan_ujian,upaya_program')
	    ->from('kemampuan_lulusan t')
	    ->join('prodi p', 't.id_prodi=p.id_prodi')
	    ->join('administrasi a', 't.id_administrasi=a.id_administrasi')
	    ->andWhere('t.id_prodi=:id', array(':id'=>$id_prodi))
        ...
        if($id_prodi == 1){
        	// footer
        	$mPDF1->SetHTMLFooter('<p style="padding-top:5px;border-top:1px solid black;width:100%;text-align:left;font-size:11px;font-weight:bold;">BAN-PT : Portofolio Fakultas/ Sekolah Tinggi, Akreditasi PS Doktor '.$data["data"]["th_akreditasi"].'</p>');
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_kemampuanlulusan_fakultas', $data, true));
        }else{
        	// footer
        	$mPDF1->SetHTMLFooter('<p style="padding-top:5px;border-top:1px solid black;width:100%;text-align:left;font-size:11px;font-weight:bold;">BAN-PT : Borang Akreditasi Program Studi Doktor '.$data["data"]["th_akreditasi"].'</p>');
        	# renderPartial (only 'view' of current controller)
