...
class DataMhs5Controller extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['DataMhs5']))
		{
			$model->attributes=$_POST['DataMhs5'];
			if($model->save()){
				if($model->lampiran){
				...
				$this->redirect(array('view','id'=>$model->id_mhs5));
		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('DataMhs5', 
			array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$model=new DataMhs5('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['DataMhs5']))
			$model->attributes=$_GET['DataMhs5'];
	 ...
	 * @return DataMhs5 the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=DataMhs5::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param DataMhs5 $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='data-mhs5-form')
		{
	...
	public function ActionSentData(){
		...
		$myHtml = $this->renderPartial('v_mhs_5',$dt,true); 
		echo $myHtml;
		Yii::app()->end(); 
		return;	
	}
	public function actionCetakPDF(){
        ...
        if($id_prodi == 1){
	        ...
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_mhs5_fakultas', $data, true));
        }else{
        	// footer
        	$mPDF1->SetHTMLFooter('<p style="padding-top:5px;border-top:1px solid black;width:100%;text-align:left;font-size:11px;font-weight:bold;">BAN-PT : Borang Akreditasi Program Studi Magister '.$th_akreditasi .'</p>');
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_mhs5', $dt, true));
        }
