...
class DataMhs6thnController extends Controller
{
	/**
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('admin','delete','update','create','index','view','admin','getdata','sentdata','cetakPDF','manajemen'),
	...
	{
		$model=new DataMhs6thn;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['DataMhs6thn']))
		{
			$model->attributes=$_POST['DataMhs6thn'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->id_mhs6thn));
		}
	...
	{
		$dataProvider=new CActiveDataProvider('DataMhs6thn');
	...
	{
		$model=new DataMhs6thn('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['DataMhs6thn']))
			$model->attributes=$_GET['DataMhs6thn'];
	 ...
	 * @return DataMhs6thn the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=DataMhs6thn::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param DataMhs6thn $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='data-mhs6thn-form')
		{
	...
	public function ActionSentData(){
		...
		$myHtml = $this->renderPartial('v_mhs_6thn',$dt,true); 
		echo $myHtml;
		Yii::app()->end(); 
		return;	
	}
	public function actionCetakPDF(){
        ...
        if($id_prodi == 1){
        	$mPDF1->SetHTMLFooter('<p style="padding-top:5px;border-top:1px solid black;width:100%;text-align:left;font-size:11px;font-weight:bold;">BAN-PT : Portofolio Fakultas/ Sekolah Tinggi, Akreditasi PS Doktoral '.$data["data"]["th_akreditasi"].'</p>');
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_mhs6_fakultas', $data, true));
        }else{
        	// footer
        	$mPDF1->SetHTMLFooter('<p style="padding-top:5px;border-top:1px solid black;width:100%;text-align:left;font-size:11px;font-weight:bold;">BAN-PT : Borang Akreditasi Program Studi Doktoral '.$th_akreditasi .'</p>');
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_mhs6', $dt, true));
        }
