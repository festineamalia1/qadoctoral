...
class DampakPenelitian_psController extends Controller
{
	/**
	...
	{
		$model=new DampakPenelitian_ps;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['DampakPenelitian_ps']))
		{
			$model->attributes=$_POST['DampakPenelitian_ps'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->id_dampakpenelitian));
		}
	...
	{	
		...
		$dataProvider=new CActiveDataProvider('DampakPenelitian_ps',
			array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$model=new DampakPenelitian_ps('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['DampakPenelitian_ps']))
			$model->attributes=$_GET['DampakPenelitian_ps'];
	 ...
	 * @return DampakPenelitian_ps the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=DampakPenelitian_ps::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param DampakPenelitian_ps $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='dampak-penelitian-ps-form')
		{
