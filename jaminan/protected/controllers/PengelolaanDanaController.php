...
class PengelolaanDanaController extends Controller
{
	...
	public function actionCetakPDFpengelolaan(){
        ...
        if($id_prodi == 1){
        	// footer
        	$mPDF1->SetHTMLFooter('<p style="padding-top:5px;border-top:1px solid black;width:100%;text-align:left;font-size:11px;font-weight:bold;">BAN-PT : Portofolio Fakultas/ Sekolah Tinggi, Akreditasi Doktoral '.$dt['thn_administrasi']['th_akreditasi'].'</p>');
	...
	public function actionSentDataPerolehan(){
		...
		if($id_prodi == 1){
		    ...
		    $data['penggunaan'] = Yii::app()->db->createCommand()
		     ...
		     $dt['tridarma'] = Yii::app()->db->createCommand()
			    ...
			    ->queryAll();
				 ...
				 $dt['operasional'] = Yii::app()->db->createCommand()
			    ->select('k.*')
			    ->from('dana_operasional k')
			    ->join('prodi p', 'k.id_prodi=p.id_prodi')
			    ->join('administrasi a','a.id_administrasi=k.id_administrasi')
			    ->andWhere('p.id_prodi=:id', array(':id'=>$id_prodi))
				->andWhere('a.id_administrasi=:id_a', array(':id_a'=>$id_administrasi))
		    ...
		    $dt['dana'] = Yii::app()->db->createCommand()
			    ->select('k.*, b.nama_dosen')
			    ->from('dana_penelitian k')
			    ->join('prodi p', 'k.id_prodi=p.id_prodi')
			    ->join('administrasi a','a.id_administrasi=k.id_administrasi')
			    ->leftjoin('data_dosen b','k.id_dosen=b.id_dosen')
			    ->andWhere('k.status=:st', array(':st'=>'penelitian'))
			    ->andWhere('p.id_prodi=:id', array(':id'=>$id_prodi))
				->andWhere('a.id_administrasi=:id_a', array(':id_a'=>$id_administrasi))
				//->andWhere('b.id_dosen=:id_b', array(':id_b'=>$id_dosen))
			    ->queryAll();
			...
			$dt['dana_pkm'] = Yii::app()->db->createCommand()
			    ...
			    ->join('prodi p', 'k.id_prodi=p.id_prodi')
			    ->join('administrasi a','a.id_administrasi=k.id_administrasi')
			    //->join('data_dosen b','b.id_dosen=k.id_dosen')
			    ->andWhere('k.status=:st', array(':st'=>'penelitian'))
			    ->andWhere('p.id_prodi=:id', array(':id'=>$id_prodi))
				->andWhere('a.id_administrasi=:id_a', array(':id_a'=>$id_administrasi))
				//->andWhere('b.id_dosen=:id_b', array(':id_b'=>$id_dosen))
	...
	public function actionSentDataPrasarana(){
		...
		if($id_prodi == 1){
		    ...
		    ->queryAll();
		     ...
		     $dt['ketersediaan'] = Yii::app()->db->createCommand()
		    ->select('k.*')
		    ->from('ketersediaan_tempat k')
		    ->join('prodi p', 'k.id_prodi=p.id_prodi')
		    ->join('administrasi a','a.id_administrasi=k.id_administrasi')
		    ->andWhere('p.id_prodi=:id', array(':id'=>$id_prodi))
			->andWhere('a.id_administrasi=:id_a', array(':id_a'=>$id_administrasi))
	...
	public function actionSentDataSarana(){
		...
		foreach($data['pustaka'] as $key => $value){
			if($value['jenis_pustaka'] == 'buku teks'){
				$data['jenis_pustaka']['buku teks'][] = $value;
			}else if($value['jenis_pustaka'] == 'jurnal nas'){
				$data['jenis_pustaka']['jurnal nas'] = $value;
			}else if($value['jenis_pustaka'] == 'jurnal inter'){
				$data['jenis_pustaka']['jurnal inter'][] = $value;
			}else if($value['jenis_pustaka'] == 'prosiding'){
				$data['jenis_pustaka']['prosiding'][] = $value;
			}else if($value['jenis_pustaka'] == 'disertasi'){
				$data['jenis_pustaka']['disertasi'][] = $value;
			}
