...
<div class="row-fluid">
	<div class="span12" style="padding:20px;">
		<form class="form-inline" method="post" id="form_himpunan">
		    ...
		    <button type="button" class="btn btn-primary getdata_himpunan"><i class="icon-arrow-right icon-white"></i> Tampil</button>
		</form>
	</div>
</div>
<div class="row-fluid">
...
$(function(){
  $(".getdata_himpunan").click(function()
  	{
  		var dataserialized = $('#form_himpunan').serialize();
  		//$("#data").hide();
  		$.ajax({
  			type: 'POST',
  			url:"<?=Yii::app()->getBaseUrl(true)?>/himpunanalumnis2/sentdata",
  			data: dataserialized,
		   	beforeSend:function(){
  ...
  $(".getdata_himpunan").trigger('click');
